"""First-order ``R0-R1-C1`` equivalent-circuit model."""
...
class CellModel_R0R1C1(CellModel):
    ...
    ``[R0, R1, C1]``.  The transition uses the fixed sampling interval passed
    to the constructor, exactly as in the MATLAB class.
    """
    ...
    def f1p(self, xold: Any, pold: Any, uold: Any, deltat: float | None = None) -> FloatArray:
        ...
        tau = p[1] * p[2]
        alpha = float(np.exp(-self.deltatfix / tau))
        adtrc = alpha * self.deltatfix / tau
        result = np.zeros((2, 3), dtype=np.float64)
        result[1, 1] = adtrc / p[1] * x[1] + (alpha - 1.0 + adtrc) * u[0]
        result[1, 2] = adtrc / p[2] * (x[1] + p[1] * u[0])
        return result
